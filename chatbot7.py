...
def detect_emotion_keywords(text):
    ...
    if emotion_scores:
        ...
        confidence = min(best_emotion[1] * 0.3, 0.9)
        return {
...
def demo():
    scenarios = [
        ...
        {"name": "Love", "input": "I'm head over heels in love!", "expected": "love"}
    ]
...
def print_help():
    ...
    print("  'I'm really angry about this'")
...
def run_demo():
    ...
    demos = [
        ...
        ("Fear", "I'm scared about tomorrow")
    ]
    ...
    for name, text in demos:
        ...
        print(f"\n{name} Test:")
        print(f"Input: '{text}'")
        print(f"Detected: {emotion_data['primary_emotion']} ({emotion_data['confidence']:.1%})")
...
def main():
    ...
    while True:
        try:
            ...
            # Display response
            print(f"\n🔍 Detected: {emotion} ({emotion_data['confidence']:.1%}) [{emotion_data.get('model', '?')}]")
